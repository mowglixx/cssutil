...
project = {
    ...
    'version': '0.2',
    'author': 'Dan Monaghan',
    'github': 'https://github.com/mowglixx/cssutil',
}
...
CSS_Selector('text-center', ['flex-wrap: wrap'])
...
for (short_bound, bound) in measurements.items():
    count = 0
    while count < 9:
        CSS_Selector(f'{short_bound}-{count}', [f'{bound}: {round(count*measuring_step, 2)}em'])
        count += 1
...
for (short_bound, bound) in measurements.items():
    ...
    while count < 9:
        for (shortdir, dir) in directions.items():
            CSS_Selector(f'{short_bound}{shortdir}-{count}', [f'{bound}-{dir}: {round(count*measuring_step, 2)}em'])
        count += 1
